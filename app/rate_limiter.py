...
class TokenBucket:
    ...
    def available(self, client_id: str, now: float | None = None) -> float:
        ...
        if not state:
            return float(self.capacity)  # client mới → xô đầy

        tokens = float(state["tokens"])
        last = float(state["ts"])
        tokens += (now - last) * self.refill_per_second
        # Chặn trên ở capacity — thiếu min() thì client im lặng một ngày tích
        # được 14.400 token rồi bắn hết trong một giây.
    ...
    def consume(self, client_id: str, now: float | None = None) -> None:
        ...
        # Ghi lại CẢ ts. Quên cập nhật mốc thời gian thì lần sau phần nạp
        # thêm được tính từ một mốc đã cũ, và xô tự đầy lại vô tội vạ.
