...
def readyz(store: ChatStore = Depends(get_store)):
    ...
    503 ở đây làm LB ngừng gửi traffic, KHÔNG làm container bị restart.
    """
    if shutdown_guard.draining:
        return JSONResponse(status_code=503, content={"status": "draining"})
    if not store.ping():
...
def chat(
    ...
    token hợp lệ đã dừng ở 401 trước khi chạm vào bất cứ dòng nào ở đây.
    """
    # Chặn TRƯỚC khi gọi LLM — tiền mất ở bước gọi LLM. Chặn sau thì vừa
    # trả tiền vừa trả lỗi cho user.
